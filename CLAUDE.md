...
**Field notes are Markdown files in `src/content/field-notes/`,** one per note, and the file name is the slug. Frontmatter carries the title, dek, topic, dates, cover and the Lab experiment or project the note came out of; every `##` heading in the body starts a section (the "on this page" rail lists them; `## Heading {#anchor}` pins an anchor). `src/lib/posts.ts` reads and renders them with `gray-matter` + `marked` and is server-only; `src/content/posts.ts` holds the `Post` type and date helpers so client components can import them. Reading time comes from the word count unless the frontmatter sets it. `draft: true` hides a note, and files starting with `_` (`_template.md`) are never read. Adding a note means adding a file — copy the template. Paragraphs still in [brackets] are Neal's to write. `next.config.ts` lists the folder in `outputFileTracingIncludes` so the RSS route can read it on Vercel. `/writing/rss.xml` is a route handler that builds the feed from the same list and takes its absolute URLs from the request, since the site has no configured domain. The masthead card offers RSS and LinkedIn rather than an email signup, because there is no list behind one yet.
